...
-- Modified Denormalized Responses table
CREATE TABLE responses_denormalized (
    response_id UUID,
    campaign_id UUID,
    participant_id UUID,
...
-- Materialized Views for Approach 2
CREATE TABLE segment_metrics_mv_2 (
    campaign_id UUID,
    question_id UUID,
    segment_value String,
    date Date,
...
CREATE MATERIALIZED VIEW segment_metrics_mv_view_2
TO segment_metrics_mv_2
AS 
SELECT 
    rd.campaign_id,
    rd.question_id,
    rd.segment_value,
    toDate(rd.created_at) as date,
    count(DISTINCT rd.response_id) as response_count,
...
WHERE NOT rd.discarded
GROUP BY 
    rd.campaign_id,
    rd.question_id,
    rd.segment_value,
    toDate(rd.created_at);